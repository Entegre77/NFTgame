...
#include <GL/glu.h>
#include "texfont.h"
#include "ou.h"					/* openuniverse addition */
...
TexFont *txfLoadFont(char *filename)
{
	...
	char fullpath[100];			/* openuniverse addition */
	...
	txf = NULL;
	sprintf(fullpath, "%s/%s", fontpath, filename);	/* openuniverse addition */
	file = fopen(fullpath, "rb");	/* openuniverse addition */
	if (file == NULL) {
...
txfGetStringMetrics(TexFont * txf,
					char *string,
					int len, int *width, int *max_ascent, int *max_descent)
{
	TexGlyphVertexInfo *tgvi;
	int w, i;
